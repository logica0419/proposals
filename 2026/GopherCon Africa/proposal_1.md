...
1. Introduction (3min)
   - Memory management (programming language): a strategy to manage variable data in memory
...
2. What memory is: Ancient memory management (1min)
   - Memory = temporary data storage for the process
     - App can use them as a KV store
...
3. Era of variables: the stack and the heap (4min)
   ...
   - High-level programming language arose
     ...
     - Programmers don't need to know the address, but the language needs to manage them
   - Invention of the stack
     - The stack was perfect to represent the nested structure of function calls & variable scope in memory space
     - Illustrated explanation of the stack
   - Partial use of direct insertion: the heap and the pointer
     - For big / unknown-sized data
     - For data used across the functions
...
5. Heap escaping in Go (5min)
   - (Pain) Placing many data in the heap = high GC cost
     - (Example) Putting all non-primitives in the heap -> over half of the variables are GC targets
   - Heap escaping
     - Put as much data in the stack as possible
     - Put necessary data in a heap; types don't matter
   - Heap escape analysis & membench
     - Heap escaping is mostly determined by the compiler; the runtime also has some influence
     - A build option to show variable analysis log
     - We can see the actual allocation result with membench
   - The structure of the heap
     - Objects of the same size are grouped in a span
     - Spans are grouped in bigger groups...
   - Zero-allocation libraries
     - A package that no data escapes to the heap in its functions
...
Since neither my university nor my part-time employer covers travel expenses for conference talks, I generally can't accept speaking opportunities unless travel support (flights and accommodation) is provided.
